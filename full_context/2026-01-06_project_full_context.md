...
Generated at: 2026-01-06 13:44:42
...
### stock-analyzer4/config/ai_prompts.yaml
...
### stock-analyzer4/config/config.yaml
        ...
        if debug_mode:
            for cmd in self.commands.values():
                ...
                if hasattr(cmd, "agent") and cmd.agent:
                    ...
                    # [Fix] Propagate to KeyManager as well to prevent client generation
                    if hasattr(cmd.agent, "key_manager") and cmd.agent.key_manager:
                        cmd.agent.key_manager.debug_mode = True
...
### stock-analyzer4/orchestrator.py
...
### stock-analyzer4/src/__init__.py
...
### stock-analyzer4/src/ai/__init__.py
...
### stock-analyzer4/src/ai/key_manager.py
...
### stock-analyzer4/src/ai/prompt_builder.py
        ...
        for path in paths_to_try:
            if os.path.exists(path):
                try:
                    ...
                    self.logger.error(f"Failed to load thresholds.yaml from {path}: {e}")
        return {}
...
"""
        ...
        market_context_paths = [
            "market_context.txt",
            os.path.join("stock-analyzer4", "market_context.txt"),
            os.path.join("config", "market_context.txt"),
            os.path.join("stock-analyzer4", "config/market_context.txt"),
        ]
        actual_context_path = None
        for p in market_context_paths:
            if os.path.exists(p):
                actual_context_path = p
                break

        if actual_context_path:
            try:
                with open(actual_context_path, "r", encoding="utf-8") as f:
                    lines = f.readlines()
                    filtered_lines = [
                        line
                        for line in lines
...
### stock-analyzer4/src/ai/response_parser.py
...
### stock-analyzer4/src/calc/__init__.py
...
### stock-analyzer4/src/calc/base.py
...
### stock-analyzer4/src/calc/engine.py
...
### stock-analyzer4/src/commands/__init__.py
...
### stock-analyzer4/src/commands/analyze.py
...
class AnalyzeCommand(BaseCommand):
    ...
    def _process_single_stock(self, row, strategy_name):
        ...
        if not cached:
            ...
            try:
                stock_data = StockAnalysisData(**row_dict)
                is_abnormal = stock_data.should_skip_analysis
                reasons = [r.value for r in stock_data.validation_flags.skip_reasons]
            except Exception:
                # フォールバック: 旧ロジックを使用
                from src.validation_engine import ValidationEngine
...
### stock-analyzer4/src/commands/base_command.py
...
### stock-analyzer4/src/commands/extract.py
...
### stock-analyzer4/src/commands/ingest.py
...
### stock-analyzer4/src/commands/reset.py
...
### stock-analyzer4/src/config_loader.py
...
class ConfigLoader:
    ...
    def _ensure_directories(self):
        ...
        # 1. Outputディレクトリの作成
        output_path = self.raw_config.get("data", {}).get("output_path")
        if output_path:
        ...
        paths_out = self.raw_config.get("paths", {}).get("output_dir")
        if paths_out and not os.path.exists(paths_out):
            os.makedirs(paths_out, exist_ok=True)
            print(f"📁 Created directory: {paths_out}")

        # 3. データベースディレクトリの作成
        db_file = self.raw_config.get("paths", {}).get("db_file")
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        paths_to_try = [
            self.config_path,
            os.path.join("stock-analyzer4", self.config_path),
        ]
        ...
        if not actual_path:
            print(f"⚠️ Config file not found in: {paths_to_try}")
            return {}

        self.config_path = actual_path  # 実際に使用したパスを保持
        try:
...
### stock-analyzer4/src/constants.py
    ...
    def cleanup_and_optimize(self, retention_days=30):
        ...
        try:
            ...
            with db_proxy.atomic():
                ...
                child_del_q = AnalysisResult.delete().where(
                    AnalysisResult.market_data << old_market_ids
                )
...
### stock-analyzer4/src/database_factory.py
...
### stock-analyzer4/src/domain/models.py
...
class StockAnalysisData(BaseModel):
    ...
    model_config = ConfigDict(extra="ignore")  # 不明なフィールドは無視
...
### stock-analyzer4/src/env_loader.py
...
### stock-analyzer4/src/fetcher/__init__.py
...
### stock-analyzer4/src/fetcher/facade.py
...
### stock-analyzer4/src/fetcher/jpx.py
...
### stock-analyzer4/src/fetcher/technical.py
...
### stock-analyzer4/src/fetcher/yahoo.py
...
class Orchestrator:
    ...
    def _execute_equity_auditor(
        ...
        for i in range(0, len(codes), batch_size):
            ...
            # equity_auditor.py は stock-analyzer4 直下にあるため、絶対パスを構築
            import os
            script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            auditor_path = os.path.join(script_dir, "equity_auditor.py")
            cmd = [
                sys.executable,
                auditor_path,
                "--mode",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Stock Analysis Orchestrator")
    parser.add_argument(
        "mode", choices=["daily", "weekly", "monthly"], help="Execution mode"
    )
...
### stock-analyzer4/src/reporter.py
...
class ResultWriter:
    ...
    def __init__(self, config):
        ...
        self.output_dir = paths.get("output_dir") or "data/output"
...
### stock-analyzer4/src/validation_engine.py
...
class ValidationEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 設定からマッピング情報をロード
        self.mapping = config.get("metadata_mapping") or {}
