...
try:
    ...
    from src.models import AnalysisResult, MarketData, RankHistory, SentinelAlert, Stock
except ImportError:
...
class TestExtractCommandExtended(unittest.TestCase):
    ...
    def test_execute_no_candidates(self):
        ...
        with patch.object(
            self.extract_cmd, "_fetch_candidates_by_code", return_value=[]
        ):
    ...
    def test_execute_success_flow(self):
        ...
        with patch.object(
            self.extract_cmd, "_fetch_candidates_logic", return_value=candidates
        ):
            with patch.object(self.extract_cmd, "_run_async_batch") as mock_run:
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    self.extract_cmd.logger.info.assert_any_call(
                        f"✅ Saved 1 tasks to: {out_path}"
                    )
    ...
    def test_run_async_batch(self, mock_sem_cls, mock_gather):
        ...
        try:
            results = loop.run_until_complete(
                self.extract_cmd._run_async_batch(candidates, "test")
            )
        finally:
            loop.close()
    ...
    def test_process_single_candidate(self):
        ...
        task, is_valid, reason = self.extract_cmd._process_single_candidate(
            row, "strategy_A"
        )
...
class TestDatabaseExtended(unittest.TestCase):
    ...
    def test_manual_migration_adds_columns(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE market_data (id INTEGER PRIMARY KEY, code TEXT, entry_date TEXT)"
        )
        conn.commit()
        conn.close()
    ...
    def test_upsert_market_data_and_cleanup(self):
        ...
        self.db.upsert_stocks(
            [{"code": "9001", "name": "Test", "sector": "S", "market": "M"}]
        )
...
class TestPromptBuilderExtended(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_config = {
            ...
            "sector_policies": {"Technology": {"ai_prompt_excludes": ["per"]}},
            "sector_risks": {"Technology": "High Volatility"},
        }

        with patch(
            "src.ai.prompt_builder.PromptBuilder._load_prompt_template"
        ) as mock_pt:
            with patch(
                "src.ai.prompt_builder.PromptBuilder._load_thresholds"
            ) as mock_th:
    ...
    def test_prepare_variables_logic(self):
        ...
        self.assertIn(
            "SPECIAL ANALYST INSTRUCTION", vars_dict_spec.get("special_instruction", "")
        )
...
class TestAnalyzerExtended(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("src.analyzer.DataProvider"), patch(
            "src.analyzer.AnalysisEngine"
        ), patch("src.analyzer.ResultWriter"), patch("src.analyzer.AIAgent"):
    ...
    def test_run_analysis_input_path(self):
        ...
        try:
            ...
            self.analyzer.logger.info.assert_any_call("Loaded 1 records from CSV.")
        finally:
            os.remove(tmp_path)
